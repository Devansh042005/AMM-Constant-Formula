...
contract AMM {
    ...
    // function to add liquidity in AMM

    function add_Liquidity(uint _amtA , uint _amtB) external {
        if(reserveA > 0 || reserveB > 0){
            require(reserveA*_amtB == reserveB*_amtA , "Invalid ratio"); // x/y = dx/dy
        }
        require(_amtA > 0 && _amtB > 0, "amount must be greater than zero");
    ...
    function remove_liquidity(uint _shares) external {
        ...
        require( shares[msg.sender] >= _shares, "not have enough");
    ...
    function swap(address _token , uint _amt) external {
        ...
        uint amtInWithFee = (_amt*997)/1000;
        ...
        _update(_tokenA.balanceOf(address(this)), _tokenB.balanceOf(address(this)));
    }
